...
const LoginPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0a0e1a] flex items-center justify-center px-4 py-10" data-testid="login-page">
      <div className="w-full max-w-5xl grid gap-6 lg:grid-cols-[0.92fr,1.08fr]">
        <div className="rounded-[2rem] border border-slate-800/50 bg-[radial-gradient(circle_at_top_left,rgba(245,158,11,0.16),transparent_32%),linear-gradient(180deg,rgba(17,24,39,0.9),rgba(9,17,30,0.96))] p-8 shadow-[0_30px_90px_rgba(2,6,23,0.45)]">
          ...
          <h1 className="studio-display text-4xl font-black text-white leading-tight">Sign into the studio dashboard you can actually pitch with.</h1>
          <p className="text-slate-400 text-sm mt-4 leading-7 max-w-xl">
            Save simulations, compare concepts, organize notes, and keep director idea-lab experiments in one workspace. This Netlify edition uses local browser accounts now and can graduate to a real API later.
          </p>
          <div className="mt-8 grid gap-3 sm:grid-cols-3">
            {[
              ["Predict", "Genre mix, OTT fit, ROI"],
              ["Package", "Export briefs and compare ideas"],
              ["Develop", "Get inspiration, budgets, and location prompts"],
            ].map(([label, desc]) => (
              <div key={label} className="rounded-2xl border border-slate-800/60 bg-slate-950/30 p-4">
                <div className="text-[11px] uppercase tracking-[0.22em] text-amber-400">{label}</div>
        ...
        <div className="w-full max-w-md lg:max-w-none lg:justify-self-end">
          ...
          <div className="bg-[#111827]/80 rounded-2xl border border-slate-800/50 p-8">
            ...
            <p className="text-slate-600 text-xs mt-2">
              Netlify-only mode currently uses local browser sign-in for a fully static deployment.
            </p>
