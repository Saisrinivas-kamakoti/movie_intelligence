...
import { Users, Play, BookOpen, ChevronRight, ChevronDown } from "lucide-react";

const DirectorSuite = ({ API }) => {
  ...
  const fetchData = async () => {
    try {
      const [presetsRes, casesRes] = await Promise.all([
        axios.get(`${API}/directors/presets`),
  ...
  const testPreset = async (presetId) => {
    ...
    try {
      const res = await axios.post(`${API}/directors/test/${presetId}`);
  ...
  return (
    <div data-testid="director-suite-view">
      <div className="mb-8">
        <h2 className="text-3xl font-black text-white tracking-tight">Director Testing Suite</h2>
        <p className="text-slate-500 mt-1">Test concepts from famous directors' styles & study real case studies</p>
      </div>
          ...
          {presets.map(preset => (
            ...
            >
              ...
              <div className="flex flex-wrap gap-1">
                {preset.concept.genres.map(g => (
                  <span key={g} className="px-2 py-0.5 bg-slate-800/60 rounded text-[10px] text-slate-400">{g}</span>
                ))}
              </div>
      ...
      {presetResult && !testing && (
        <div className="mb-10 bg-[#111827]/80 rounded-2xl border border-amber-500/20 p-6" data-testid="preset-result">
          ...
          <div className="space-y-1">
            {(presetResult.prediction.recommendations || []).map((rec, i) => (
              <div key={i} className="text-slate-300 text-xs bg-slate-900/30 rounded p-2">{rec}</div>
            ))}
          </div>
        </div>
      )}
                ...
                <div className="p-5 pt-0 border-t border-slate-800/30">
                  ...
                  <div className="mb-4">
                    ...
                    <ul className="space-y-1">
                      {study.key_insights.map((insight, i) => (
